...
async def begin(message: types.Message):
    start_buttons = ["Джинсы", "Что-то другое"]
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(*start_buttons)

    await message.answer("Hey!", reply_markup=keyboard)

@dp.message_handler(Text(equals="Джинсы"))
async def get_jeans(message: types.Message):
    await message.answer(get_content(html.text))
...
#
# content = first_progect.get_content(html.text)
